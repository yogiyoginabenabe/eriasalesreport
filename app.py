...
2. 「おはようございます！」から始め、直近期間への労い→数字で確認できる現在地→その数字の背景にある日報の具体的な接客・成約・工夫→週末の勝負どころ、の順に3〜5段落で展開。
   数字だけを列挙して段落を終えない。必ず日報の定性的事実を1〜3件つなぎ、「現場で何が起きてその数字につながっているか」を示す。
   【渡邊AMからの追加情報】に重点施策や背景がある場合は、現在地と日報の事実に自然につなげる。
3. 受注・座数・客数・CVR・客単価・品数を横断し、組み合わせから読める現場課題を慎重に説明する。
   例：座数が弱ければ接客機会、CVRが弱ければ成約、客単価や品数が弱ければセット提案が確認点。ただし断定せず「可能性」「次に確認したい点」とする。
4. 【SM日報レスポンス】
   選定済み全員に対し「●氏名さん（店舗名）：」で返信。
   必ず「具体的な接客・成約・工夫→なぜ価値があるか→今日どう再現するか→次回以降も続けてほしい強み」の順に1人2〜4文でつなぐ。
   「今日もこの強みでいこう」「次もこの行動を続けよう」という、本人が承認と期待を感じられるメッセージを自然に入れる。
5. ----------------------------------
   🌸本日のブーストアクション🌸
   ----------------------------------
   根拠から導ける行動を3〜4項目。「①行動タイトル」「└ 店頭で誰が何をするか」の形で具体化。
   土日の全員共有は「ミーティング」ではなく「朝礼」と表現する。
   商品を組み合わせる提案は「複数点販売」ではなく「セット販売」と表現する。
